...
def check_style(config):
    ...
    if child.returncode != 0 or len(extract_stdout(child)) != 0:
        ...
        coding_style_grade = min(config.coding_style, (1.0 * max(min(config.grade, config.tests_points), 0) /
...
def run_task(config, task):
    ...
    log('----- running task {} - grade {:g}/{:g}\n'.format(task.id,
                                                           min(task.grade, task.points), task.points))
    unindent_log()
...
def checker_run():
    ...
    log('For good implementations, you can receive up to 10 extra points')
